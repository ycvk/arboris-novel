...
class DraftService:
    ...
    async def save_draft(
        self, project_id: str, draft_data: SaveDraftRequest
    ) -> BlueprintDraft:
        """保存蓝图草稿.
        ...
        logger.info(
            "保存蓝图草稿",
            extra={"project_id": project_id, "current_stage": draft_data.current_stage},
        )
        ...
        logger.info(
            f"草稿保存成功 - project_id={project_id}, current_stage={draft_data.current_stage}"
        )
    ...
    async def get_draft(self, project_id: str) -> BlueprintDraft | None:
        """获取蓝图草稿.
        ...
        try:
            draft = BlueprintDraft(
                ...
                stage1=Stage1Data(**draft_data["stage1"])
                if draft_data.get("stage1")
                else None,
                stage2=Stage2Data(**draft_data["stage2"])
                if draft_data.get("stage2")
                else None,
                stage3=Stage3Data(**draft_data["stage3"])
                if draft_data.get("stage3")
                else None,
                stage4=Stage4Data(**draft_data["stage4"])
                if draft_data.get("stage4")
                else None,
                updated_at=draft_data.get("updated_at"),
            )
            logger.info(
                f"草稿获取成功 - project_id={project_id}, current_stage={draft.current_stage}"
            )
            return draft
