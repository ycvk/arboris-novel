...
class SimpleBlueprintService:
    ...
    async def generate_blueprint(
        ...
        try:
            # 简化的实现：一次性生成所有数据
            system_prompt = "你是一个网络小说创作助手。请基于对话历史生成一个简单的小说蓝图。"
            user_prompt = f"对话历史：{json.dumps(conversation_history, ensure_ascii=False)}"
            ...
            logger.error("蓝图生成失败", extra={"error": str(e), "project_id": project_id})
            # 返回默认蓝图
            return {
                "title": "默认蓝图",
                "genre": "默认类型",
        ...
        try:
            ...
            if project:
                ...
                if not hasattr(project, 'metadata'):
                    project.metadata = {}
                project.metadata['blueprint_data'] = json.dumps(blueprint, ensure_ascii=False)
