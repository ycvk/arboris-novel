...
class UpdateLogService:
    ...
    async def list_logs(self, limit: int | None = None) -> list[UpdateLog]:
        if limit is None:
    ...
    async def update_log(
        self, log_id: int, *, content: str | None = None, is_pinned: bool | None = None
    ) -> UpdateLog:
        log = await self.repo.get(id=log_id)
        if not log:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="更新记录不存在"
            )
